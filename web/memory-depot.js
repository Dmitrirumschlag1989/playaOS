...
function normalizeMemory(memory) {
  return {
    ...
    author: memory.uploadedBy || memory.author || 'Anonymous',
    day: memory.day || '',
    event: memory.eventName || memory.event || '',
    sync: 'cloud',
  };
}

function contentUrl(memory) {
  if (memory.contentUrl) return memory.contentUrl;
  if (API && memory.id) {
    return `${API.base}/memories/${encodeURIComponent(memory.id)}/content`;
  }
  return memory.url || '';
}
...
function render() {
  ...
  const filtered = memories.filter((memory) => {
    ...
    return matchesDay && matchesKind && matchesSearch;
  });

  $('#memoryGrid').innerHTML = filtered.length
    ? filtered.map((memory) => {
        ...
        return `
          <article class="memoryCard card">
            <div class="memoryMedia">${media}</div>
            <div class="memoryInfo">
              <strong>${esc(memory.caption || 'Untitled memory')}</strong>
              <div class="meta">
                ${esc(memory.author)}${memory.day ? ` · ${fmtDay(memory.day)}` : ''}
              </div>
              ${memory.location ? `<div class="meta">📍 ${esc(memory.location)}</div>` : ''}
              ${memory.event ? `<span class="tag">${esc(memory.event)}</span>` : ''}
              <span class="tag">☁ shared</span>
            </div>
          </article>
        `;
      }).join('')
    : '<div class="card emptyMemory">No memories yet. Be the first one in.</div>';
}

function populate() {
  const options = days
    .map((day) => `<option value="${day}">${fmtDay(day)}</option>`)
    .join('');
...
function openFiles(files) {
  ...
  $('#preview').innerHTML = pending.map((file) => {
    if (file.type.startsWith('image/')) {
      return `<img class="previewImg" src="${URL.createObjectURL(file)}" alt="Preview">`;
    }

    return `<div class="card">🎥 ${esc(file.name)}</div>`;
  }).join('');
...
async function loadCloud() {
  ...
  try {
    const response = await API.request('/memories/feed', {
      cache: 'no-store',
    });
...
async function uploadCloud(file, metadata) {
  ...
  if (file.size > session.maxBytes) {
    throw new Error(`${file.name} exceeds the ${Math.round(session.maxBytes / 1024 / 1024)} MB upload limit.`);
  }
...
async function submit(event) {
  ...
  try {
    ...
    alert(
      `Memory Depot could not upload this item yet.\n\n${error.message}\n\nYour files were not discarded.`
    );
  } finally {
...
['memorySearch', 'memoryDay', 'memoryKind'].forEach((id) => {
