...
function render() {
  const query=$('#memorySearch').value.toLowerCase(), selectedDay=$('#memoryDay').value, selectedKind=$('#memoryKind').value;
  ...
  $('#memoryGrid').innerHTML=filtered.length?filtered.map((memory)=>{
    ...
    const media=memory.kind==='photo'&&src
      ? `<button class="mediaOpen" type="button" data-memory-index="${index}" aria-label="Open memory"><img src="${esc(src)}" alt="${esc(memory.caption||'Playa memory')}" loading="lazy"></button>`
      : memory.kind==='video'&&src
        ? `<button class="mediaOpen videoThumb" type="button" data-memory-index="${index}" aria-label="Play memory"><video src="${esc(src)}" preload="metadata" muted playsinline></video><span class="playBadge">▶</span></button>`
...
function openFiles(files) {
  ...
  $('#preview').innerHTML=pending.map((file)=>file.type.startsWith('image/')
    ? `<div class="previewItem"><img class="previewImg" src="${URL.createObjectURL(file)}" alt="Preview"><small>${esc(file.name)}</small></div>`
    : `<div class="previewItem card">🎥 ${esc(file.name)}</div>`).join('');
...
async function uploadCloud(file,metadata) {
  ...
  const sessionResponse=await API.request('/memories/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contentType:file.type,originalName:file.name,...metadata})});
  ...
  if(!uploadResponse.ok){let detail=`HTTP ${uploadResponse.status}`;try{const body=await uploadResponse.json();detail=body.error||detail;}catch(_){}throw new Error(`Upload failed: ${detail}`);}
...
async function submit(event) {
  ...
  const metadata={uploadedBy,caption:$('#memoryCaption').value.trim(),day:$('#memoryDayForm').value,location:$('#memoryLocation').value.trim(),eventName:$('#memoryEvent').value.trim()};
  ...
  try {
    for(let i=0;i<pending.length;i++){
      ...
      button.textContent=`Uploading ${i+1} of ${total}…`;
...
function openViewer(index) {
  ...
  const src=contentUrl(memory);
  const media=memory.kind==='video'
    ? `<video src="${esc(src)}" controls autoplay playsinline></video>`
    : `<img src="${esc(src)}" alt="${esc(memory.caption||'Playa memory')}">`;
  $('#viewerMedia').innerHTML=media;
  $('#viewerInfo').innerHTML=`<strong>${esc(memory.caption||'Untitled memory')}</strong><div>${esc(memory.author)}${memory.day?` · ${fmtDay(memory.day)}`:''}</div>${memory.location?`<div>📍 ${esc(memory.location)}</div>`:''}${memory.event?`<span class="tag">${esc(memory.event)}</span>`:''}`;
  $('#viewerPrev').disabled=index<=0;
  $('#viewerNext').disabled=index>=memories.length-1;
  if(!$('#memoryViewer').open)$('#memoryViewer').showModal();
}

function stepViewer(direction) {
  const next=viewerIndex+direction;
  if(next>=0&&next<memories.length)openViewer(next);
}
...
$('#memoryGrid').addEventListener('click',(event)=>{
  const target=event.target.closest('[data-memory-index]');
  if(target)openViewer(Number(target.dataset.memoryIndex));
});
$('#closeViewer').onclick=()=>$('#memoryViewer').close();
...
document.addEventListener('keydown',(event)=>{if(!$('#memoryViewer').open)return;if(event.key==='Escape')$('#memoryViewer').close();if(event.key==='ArrowLeft')stepViewer(-1);if(event.key==='ArrowRight')stepViewer(1);});
