...
(()=>{
  ...
  const campLocation=camp=>{if(!camp)return '';if(typeof camp.location_string==='string'&&camp.location_string.trim())return camp.location_string.trim();const l=camp.location;if(typeof l==='string')return l.trim();if(l&&typeof l==='object'){if(l.location_string)return String(l.location_string).trim();const front=l.frontage||'',inter=l.intersection||'',type=l.intersection_type||'&';if(front&&inter)return `${front} ${type} ${inter}`}return ''};
  const normalize=(raw,camps,seedMap)=>{
    const byUid=new Map(camps.map(c=>[c.uid,c])),out=[];
    for(const e of asArray(raw)){
      const occ=Array.isArray(e.occurrence_set)?e.occurrence_set:[],occurrences=occ.length?occ:[{start_time:e.start_time,end_time:e.end_time}],camp=byUid.get(e.hosted_by_camp)||null,location=campLocation(camp)||e.other_location||'';
  ...
  window.fetch=async(input,init)=>{
    ...
    try{
      ...
      const normalized=normalize(events,asArray(camps),seedMap);if(normalized.length)return new Response(JSON.stringify({schema_version:'2.0.0',source:'Burning Man Public API',events:normalized}),{status:200,headers:{'Content-Type':'application/json'}});
