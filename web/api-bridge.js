...
(()=>{
  ...
  const normalize=(raw,camps,seedMap)=>{
    ...
    for(const c of camps){const uid=idString(c?.uid||c?.id||c?.camp_uid);if(uid)byUid.set(uid,c);const name=campName(c).toLowerCase();if(name)byName.set(name,c)}
    for(const e of asArray(raw)){
      ...
      const hostedName=cleanString(e.hosted_by_camp_name||e.camp_name||e.hosted_by_camp?.name).toLowerCase();
      const camp=byUid.get(hostedUid)||byName.get(hostedName)||null;
      const location=campLocation(camp)||eventLocation(e);
