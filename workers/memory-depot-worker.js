...
function safeName(name = 'upload') {
  return String(name).toLowerCase().replace(/[^a-z0-9._-]+/g, '-').slice(-120);
}
...
function extension(file) {
  ...
  return type.includes('/') ? `.${type.split('/')[1].replace(/[^a-z0-9]/g, '') || 'bin'}` : '.bin';
}

export async function handleMemoryDepot(request, env) {
  const url = new URL(request.url);
  if (!url.pathname === '/memories') return null;
  ...
  if (request.method === 'GET' && !id) {
    ...
    for (const object of listed.objects) {
      const item = await env.MEMORIES.get(object.key);
      if (!item) continue;
      try {
        memories.push(await item.json());
      } catch (_) {}
    }
  ...
  if (request.method === 'GET' && id && isContent) {
    const media = await env.MEMORIES.get(`media/${id}`);
    if (!media) return json({ error: 'Memory not found' }, 404, origin);
    const headers = new Headers(cors(origin));
    headers.set('Content-Type', media.httpMetadata?.contentType || 'application/octet-stream');
    headers.set('Cache-Control', 'public, max-age=31536000, immutable');
    if (media.httpEtag) headers.set('ETag', media.httpEtag);
  ...
  if (request.method === 'POST' && !id) {
    ...
    const memory = {
      ...
      caption: String(form.get('caption') || '').trim().slice(0, 1000),
      day: String(form.get('day') || '').trim().slice(0, 20),
      location: String(form.get('location') || '').trim().slice(0, 160),
      event: String(form.get('event') || '').trim().slice(0, 200),
      created_at,
      url: `${url.origin}/memories/${idValue}/content`,
      original_name: safeName(file.name),
    };

    await env.MEMORIES.put(`media/${idValue}`, file.stream(), {
      httpMetadata: { contentType: file.type || 'application/octet-stream' },
      customMetadata: { kind, original_name: safeName(file.name) },
    });
    await env.MEMORIES.put(`meta/${idValue}.json`, JSON.stringify(memory), {
  ...
  if (request.method === 'DELETE' && id) {
    await env.MEMORIES.delete(`media/${id}`);
    await env.MEMORIES.delete(`meta/${id}.json`);
