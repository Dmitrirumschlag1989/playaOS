...
function cors(origin) {
  return {
    'Access-Control-Allow-Origin': ALLOWED_ORIGINS.has(origin) ? origin : 'https://dmitrirumschlag1989.github.io',
...
function extension(contentType) {
  const known = { 'image/jpeg':'jpg','image/png':'png','image/webp':'webp','image/gif':'gif','image/heic':'heic','image/heif':'heif','video/mp4':'mp4','video/quicktime':'mov','video/webm':'webm','video/mpeg':'mpeg','video/ogg':'ogv' };
...
async function listMemories(r2, limit = PAGE_LIMIT, cursor) {
  ...
  for (const object of listed.objects) {
    ...
    const item = await readJsonObject(r2, object.key); if (item) memories.push(item);
  }
  memories.sort((a, b) => String(b.uploadedAt).localeCompare(String(a.uploadedAt)));
  return { memories: memories.slice(0, limit), cursor: listed.truncated ? listed.cursor : null };
}
...
export async function handleMemoryDepot(request, env) {
  ...
  const r2 = bucket(env);
  if (!r2) return json({ error: 'Memory Depot R2 binding is not configured', expectedBinding: 'Memories' }, 500, origin);
  ...
  if (request.method === 'POST' && url.pathname === '/memories/upload') {
    let payload = {}; const contentType = request.headers.get('Content-Type') || '';
    if (contentType.includes('application/json')) { try { payload = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400, origin); } }
    const requestedType = safeText(payload.contentType, 120); const kind = mediaKind(requestedType);
    if (!kind) return json({ error: 'A supported image or video contentType is required' }, 415, origin);
    const id = crypto.randomUUID(); const token = crypto.randomUUID().replace(/-/g, ''); const createdAt = new Date().toISOString();
    const pending = {
      id, token, contentType: requestedType, kind, expiresAt: Date.now() + UPLOAD_TTL_SECONDS * 1000, createdAt,
      metadata: {
        uploadedBy: safeText(payload.uploadedBy, 80), caption: safeText(payload.caption, 1000), day: safeText(payload.day, 20),
        eventId: safeText(payload.eventId, 120), eventName: safeText(payload.eventName, 200), location: safeText(payload.location, 160),
  ...
  if (request.method === 'PUT' && uploadMatch) {
    const id = decodeURIComponent(uploadMatch[1]); const token = url.searchParams.get('token') || request.headers.get('X-Upload-Token');
    ...
    if (Date.now() > pending.expiresAt) { await r2.delete(uploadKey(token)); return json({ error: 'Upload session expired' }, 410, origin); }
    const requestType = request.headers.get('Content-Type') || pending.contentType;
    if (requestType !== pending.contentType) return json({ error: 'Content-Type does not match upload authorization' }, 400, origin);
    const contentLength = Number(request.headers.get('Content-Length') || 0);
    if (contentLength && contentLength > MAX_UPLOAD_BYTES) return json({ error: 'File exceeds 100 MB limit' }, 413, origin);
    ...
    const memory = {
      id, key, type: pending.kind, url: `${url.origin}/memories/${encodeURIComponent(id)}`,
      contentUrl: `${url.origin}/memories/${encodeURIComponent(id)}/content`, uploadedAt: new Date().toISOString(),
      uploadedBy: pending.metadata.uploadedBy, caption: pending.metadata.caption, day: pending.metadata.day,
      eventId: pending.metadata.eventId, eventName: pending.metadata.eventName, location: pending.metadata.location, originalName: pending.originalName,
    };
    await r2.put(metaKey(id, createdDate), JSON.stringify(memory), { httpMetadata: { contentType: 'application/json; charset=utf-8' } });
    await r2.delete(uploadKey(token)); return json(memory, 201, origin);
  }
  ...
  if (request.method === 'GET') {
    ...
    if (match) {
      ...
      if (!memory) return json({ error: 'Memory not found' }, 404, origin); return json(memory, 200, origin);
    }
  }
  ...
  if (request.method === 'GET') {
    ...
    if (match) {
      ...
      const object = await r2.get(memory.key); if (!object) return json({ error: 'Memory content not found' }, 404, origin);
      const headers = new Headers(cors(origin)); headers.set('Content-Type', object.httpMetadata?.contentType || 'application/octet-stream'); headers.set('Cache-Control', 'private, max-age=3600');
      if (object.httpEtag) headers.set('ETag', object.httpEtag); return new Response(object.body, { headers });
    }
  }
  ...
  if (request.method === 'DELETE') {
    ...
    if (match) {
      const memory = (await listMemories(r2, 1000)).memories.find((item) => item.id === decodeURIComponent(match[1]));
      if (!memory) return json({ error: 'Memory not found' }, 404, origin);
      await r2.delete(memory.key); await r2.delete(memory.key.replace(/\.[^.]+$/, '.json')); return json({ ok: true, id: memory.id }, 200, origin);
    }
  }
